...
Task ID: 4
Agent: Main
Task: Fix auth login error na Vercel produkciji

Work Log:
- Identifikovan problem: auth cookies nisu bile podešene za HTTPS produkciju
- Ažurirana auth.ts konfiguracija:
  - Dodata `isProduction` provera
  - Cookies koriste `__Secure-` i `__Host-` prefix u produkciji
  - `secure: true` za HTTPS
  - Dodat `pkceCodeVerifier` cookie
- Push na GitHub nije uspeo zbog autentifikacije - korisnik treba ručno da push-uje

Stage Summary:
- Auth popravljena za produkciju
- Korisnik treba da:
  1. Push-uje kod na GitHub
  2. Postavi DATABASE_URL i NEXTAUTH_SECRET environment variables na Vercelu
  3. Pozove /api/seed endpoint da kreira admin korisnika
